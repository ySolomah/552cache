...
#define RPT_TABLE_SIZE 64
#define NUM_ALLOWED_PREFETCH 4
#define INDEX_TABLE_OPENEND 128
#define HISTORY_TABLE_OPENEND 2048

struct rpt_t * rptTable;
	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  if (!cp->data)
    fatal("out of virtual memory");
/* TEST */
//rptTable = (rpt_t *) malloc(RPT_TABLE_SIZE * sizeof(rpt_t));
  printf("Size is: %d\n", cp->prefetch_type);
  if(cp->prefetch_type != 0) {
  cp->rptTable = (rpt_t *) calloc(1, cp->prefetch_type * sizeof(rpt_t));
  rptTable = cp->rptTable;
for (int i = 0; i < cp->prefetch_type; i++) {
	printf("1\n");
	rptTable[i].tag = 0;
	rptTable[i].prevAddr = 0;
	rptTable[i].prevAddr = NULL;
	rptTable[i].tag = NULL;
	rptTable[i].prevAddr = -1;
	rptTable[i].stride = 0;
	rptTable[i].state = -1; // 0 for init, 1 for trans, 2 for no-pred, 3 for steady
}
printf("hello\n");
}
...
//#define INDEX_TABLE_OPENEND 128
//#define HISTORY_TABLE_OPENEND 1024
;
  cp->nodeCount = 0;
  cp->indexTable = (openend_index **) calloc(1, INDEX_TABLE_OPENEND * sizeof(openend_index *));
  cp-> historyTable = (openend_entry *) calloc(1, HISTORY_TABLE_OPENEND * sizeof(openend_entry));
  for(int i = 0; i < INDEX_TABLE_OPENEND; i++) {
//	cp->indexTable[i] = NULL;
	cp->indexTable[i].PC = 0;
	cp->indexTable[i].myHead = 0;
  }
  for(int i = 0; i < HISTORY_TABLE_OPENEND; i++) {
	cp->historyTable[i].tag = 0;
	cp->historyTable[i].address = 0;
        cp->historyTable[i].next = -1;	
	cp->historyTable[i].index = -1;
  }
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
	if(addr == NULL || cp == NULL) { return; }
	int blockSize = cp->bsize;
	int preFetchAddr = addr + blockSize;	
	md_addr_t set_val = CACHE_SET(cp, preFetchAddr);
	md_addr_t tag_val = CACHE_TAG(cp, preFetchAddr);
	md_addr_t correctedAddr = CACHE_MK_BADDR(cp, tag_val, set_val);
	if(cache_probe(cp, correctedAddr)) {
		return;
	}
	cache_access(cp, Read, correctedAddr, NULL, blockSize, NULL, NULL, NULL, 1);
}

void preFetch(struct cache_t *cp, md_addr_t addr);
void initializeState(rpt_t * tableSlot, md_addr_t tag, md_addr_t addr);
void updateState(rpt_t * tableSlot, md_addr_t addr, int newStride, int oldStride, int state);


void addToListOpenend(struct cache_t * cp, int tableIndex, md_addr_t tag, md_addr_t addr, md_addr_t PC) {

	cp->historyTable[cp->nodeCount % HISTORY_TABLE_OPENEND].tag = tag;
	cp->historyTable[cp->nodeCount % HISTORY_TABLE_OPENEND].address = addr;
	cp->historyTable[cp->nodeCount % HISTORY_TABLE_OPENEND].next = cp->indexTable[tableIndex].myHead;
	cp->historyTable[cp->nodeCount % HISTORY_TABLE_OPENEND].index = tableIndex;
	//printf("My next: %d, index %d\n", cp->historyTable[cp->nodeCount % HISTORY_TABLE_OPENEND].next, cp->historyTable[cp->nodeCount % HISTORY_TABLE_OPENEND].index);
	       
	cp->indexTable[tableIndex].myHead = cp->nodeCount;
	cp->indexTable[tableIndex].PC = PC;	
        cp->nodeCount = cp->nodeCount + 1;
}	

void prefetchOpenend(struct cache_t * cp, int addr, int index) {
	int historyIndex = cp->indexTable[index].myHead;
	int tempArray[HISTORY_TABLE_OPENEND+1];
	int i = 0;
	while(cp->historyTable[historyIndex].index == index && i != HISTORY_TABLE_OPENEND) {
		//printf("index: %d, ourIndex: %d, i: %d\n", index, cp->historyTable[historyIndex].index, i);
		//printf("Temparray at i: %d\n", tempArray[i]);
		md_addr_t myTag = cp->historyTable[historyIndex % HISTORY_TABLE_OPENEND].tag;
		tempArray[i] = cp->historyTable[historyIndex % HISTORY_TABLE_OPENEND].next % HISTORY_TABLE_OPENEND;
		historyIndex = tempArray[i];
		md_addr_t nextTag = cp->historyTable[historyIndex % HISTORY_TABLE_OPENEND].tag;
		i++;
		if(myTag != nextTag) {
			break;
		}
	...
	if(i <= 0) {
		printf("Error, shouldn't have gotten here %d\n", index);
	} else if(i == 1) {
		preFetch(cp, cp->historyTable[tempArray[0]].address);
		printf("In i=1\n");
	} else if(i == 2 || i == 3) {
		printf("In i=2,3\n");
		preFetch(cp, cp->historyTable[tempArray[0]].address + cp->historyTable[tempArray[0]].address - cp->historyTable[tempArray[1]].address);
	} else { // i >= 4
		int deltaOne = cp->historyTable[tempArray[0]].address - cp->historyTable[tempArray[1]].address;
		int deltaTwo = cp->historyTable[tempArray[1]].address - cp->historyTable[tempArray[2]].address;

		int candidateOne = 2;
		int candidateTwo = 3;
		int candidateThree = 4;
		while(candidateThree < i) {
			int deltaOneCandidate = cp->historyTable[candidateOne].address - cp->historyTable[candidateTwo].address;
			int deltaTwoCandidate = cp->historyTable[candidateTwo].address - cp->historyTable[candidateThree].address;
			if(deltaOne == deltaOneCandidate && deltaTwo == deltaTwoCandidate) {
				for(int j = candidateOne; j > 0 && j > candidateOne -4; j--) {
					printf("Prefetching");
					preFetch(cp, cp->historyTable[j-1].address - cp->historyTable[j].address + cp->historyTable[tempArray[0]].address);
				}
				return;
			}
			candidateThree++;
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
 	md_addr_t PC = get_PC();
	int offsetPCIndex = log_base2(sizeof(md_inst_t));
	int offsetTagIndex = log_base2(sizeof(md_inst_t)) + log_base2(cp->prefetch_type);
	int index = (PC >> offsetPCIndex) % INDEX_TABLE_OPENEND;
	//printf("index val %d\n", index);
	md_addr_t tagVal = (PC >> offsetTagIndex);
	md_addr_t addressVal = addr;
	addToListOpenend(cp, index, tagVal, addressVal, PC);
	//prefetchOpenend(cp, addr, index);
}

/* BEGIN STRIDE SETUP */

//#define RPT_TABLE_SIZE 64
//#define NUM_ALLOWED PREFETCH 4;
/*
...
//rpt_t * rptTable =  malloc(RPT_TABLE_SIZE * sizeof(struct rpt_t));
rpt_t * rptTable[RPT_TABLE_SIZE];
for (int i = 0; i < RPT_TABLE_SIZE; i++) {
	rptTable[i]->tag = 0;
	rptTable[i]->prevAddr = 0;
	rptTable[i]->prevAddr = NULL;
	rptTable[i]->tag = NULL;
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	int offsetPCIndex = log_base2(sizeof(md_inst_t));
	int offsetTagIndex = log_base2(sizeof(md_inst_t)) + log_base2(cp->prefetch_type);
	md_addr_t index = (PC >> offsetPCIndex) % cp->prefetch_type;
	md_addr_t tagVal = (PC >> offsetTagIndex);
	md_addr_t addressVal = addr;
	rpt_t * tableSlot = &(rptTable[index]);
	md_addr_t prevAddr = tableSlot->prevAddr;
	int stride = tableSlot->stride;
	int newStride = addr - prevAddr;
	int oldStride = tableSlot->stride;
	int state = tableSlot->state;
	if(tableSlot->tag != tagVal || tableSlot->state == -1) {
		//printf("a\n");
		initializeState(tableSlot, tagVal, addr);
		//printf("b\n");
	} else {
		updateState(tableSlot, addressVal, newStride, oldStride, state);
	}
	//printf("Entering loop\n");
	//printf("table slot val: %d\n", tableSlot->state);
	for(int i = 1; (tableSlot->state != 2 && i <= NUM_ALLOWED_PREFETCH); i++) {
		//printf("Prefetching\n");
		preFetch(cp, (tableSlot->prevAddr + (tableSlot->stride * i)));
	}
	return;
}
...
void preFetch(struct cache_t *cp, md_addr_t addr) {
	...
	//printf("Accessing: %d\n", correctedAddr);
	cache_access(cp, Read, correctedAddr, NULL, cp->bsize, NULL, NULL, NULL, 1);
}
...
void updateState(rpt_t * tableSlot, md_addr_t addr, int newStride, int oldStride, int state) {
	// 0 for init, 1 for trans, 2 for no-pred, 3 for steady
	int changeState = (newStride != oldStride) ? 1 : 0;
	int finalState = (state * 2) + changeState;
	switch(finalState) {
		case(0) : // Init -> Steady (same stride)
			tableSlot->state = 3;
			tableSlot->stride = oldStride;
			break;
		case(1) : // Init -> Transient (diff stride)
			tableSlot->state = 1;
			tableSlot->stride = newStride;
			break;
		case(2) : // Trans -> Steady (same stride)
			tableSlot->state = 3;
			tableSlot->stride = oldStride;
			break;
		case(3) : // Trans -> NoPred (diff stride)
			tableSlot->state = 2;
			tableSlot->stride = newStride;
			break;
		case(4) : // NoPred -> Trans (same stride)
			tableSlot->state = 1;
			tableSlot->stride = oldStride;
			break;
		case(5) : // NoPred -> NoPred (diff stride)
			tableSlot->state = 2;
			tableSlot->stride = newStride;
			break;
		case(6) : // Steady -> Steady (same stride)
			tableSlot->state = 3;
			tableSlot->stride = oldStride;
			break;
		case(7) : // Steady -> Init (diff stride)
			tableSlot->state = 0; 
			tableSlot->stride = oldStride;
			break;
	}
